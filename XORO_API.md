...
- **`createNewBill` proven from a script 2026-09-21:** bill `CA-B002020` (DHL Express Canada, 09/10/2026) — two untaxed expense lines (2251 VAT NL Paid 1,324.68 / 2245 VAT UK Paid 1,157.64) + `TaxAdjItemArr` GST 2,703.41 → GL posted 2251 / 2245 / **2226 - GST/HST Payable** Dr, 2100 Cr 5,185.73. Response: `{"Result": true, "Message": "Bill CA-B002020 created successfully", "Data": {billHeader, billExpenseLineArr...}}` with the new `BillId` on each line.
- **Reads (captured from the Bill page load):** `BillWebMethods.getBillObjFromBillId` `{"billId": "2021"}` → `Data: {billHeader, billExpenseLineArr, billItemLineArr, billPaymentLineArr, ...}` — the full object incl. `TaxAdjItemArr`, `BillPaid`, `StatusName` ("Open"/"Paid"), and one `billPaymentLineArr` entry per payment (`BillPaymentNumber` e.g. `B001681`, `AmountPaid`, `PaymentMethod` e.g. "American Express", `StatusName` "Cleared"). Also `PaymentWebMethods.getPaymentTermsInfoFromId` `{"Id": "1318", "TxnDate": "09-10-2026"}` and `VendorWebMethods.getVendorDataMinfromVendorId` `{"id": "387", "toTxnType": "101"}`. The public `Xerp/bill/getbill` read returns `TotalTaxAmt: 0.0` for every bill and no `TaxAdjItemArr` — use `getBillObjFromBillId` for tax detail.
- **Bill payment: not yet captured.** Paying a bill from the UI posts a "Bill Payment" GL pair (Dr 2100 / Cr the paying account, memo `|Bill# CA-B00xxxx Amount Paid n`), but the save request wasn't caught in DevTools (likely a different service name / in-page navigation) — `OutgoingPaymentWebMethods.createOutgoingPayment` guesses are unsafe (`SimulationFlag` is not a dry-run). Until captured, bill payments stay manual.
