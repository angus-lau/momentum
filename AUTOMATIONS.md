...
- **Pass 2 — DHL invoices:**
  ...
  4. **GST / EU VAT / UK VAT bill (API, verified 2026-09-21):** one Vendor Bill on **DHL Express Canada (387)** dated the statement closing date, store CA, AP 2100, term "Credit Card" (1318): expense lines **2251 - VAT NL - Paid** = EU VAT total and **2245 - VAT UK - Paid** = UK VAT total (no line tax), plus a **tax adjustment** `GST Purchase 5%` (item 110) = the GST total — so the bill total equals the "GST, EU, UK VAT" statement line. Created via `BillWebMethods.createNewBill` (payload shape in `XORO_API.md`); Aug 2026 → `CA-B002020`, 5,185.73, GL confirmed (2251 / 2245 / 2226 GST-HST Payable Dr, 2100 Cr). Vendor bill # convention `DHL GST-VAT <YYYY-MM-DD>`. Not yet a script — built inline this month; fold into the DHL flow next.
...
- **Verified:** Aug 2026 (statement 09/10/2026) — 31 DHL charges, 31/31 matched to MyBill invoices uniquely by amount and every one charged on its due date (or +1); 31 PDFs downloaded in ~1 min; parsed total $10,457.37 = the exact gap on rec 979; two statements posted (25 + 4 lines) ending at −15,853.54; GST/VAT bill `CA-B002020` created by API. Note: a manually-made duplicate of that bill (`CA-B002018`, also paid from 2106) exists for Aug — one pair needs voiding before rec 979 can balance.
